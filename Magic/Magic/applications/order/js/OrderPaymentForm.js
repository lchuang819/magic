OrderPaymentForm = Ext.extend(Ext.magic.MagicForm, {
	id:'OrderPaymentForm',
	title:'付款单',
	labelWidth:100,
	labelAlign:"right",
...
//		 		});
		    	 ...
		    	 var colModel = [
		 		                {id:'productId',header: "付款账户编号", width:60, sortable: true, locked:false, dataIndex: 'productId', editor: textField},
		 		                {header: "付款账户名称", sortable: true, width:100,dataIndex: 'brandName', editor: textField},
		 		                {header: "金额", sortable: true, width:30, dataIndex: 'quantity', editor: numberField},
		 		                {header: "####", sortable: true, width:30, dataIndex: 'unitPrice', editor: numberField},
		 		                {header: "####", sortable: true, width:30, dataIndex: 'totalPrice', editor: numberField},
...
//		    },
		...
		//
		    ...
		    onAdd : function(btn, ev) {
		        ...
		        var p = new Magic.product.ProductGrid({id:'OrderPaymentFormProductGrid'});
...
//		        var rec = this.store.getAt(index[0]);
		...
		this.buttons = [{
				            ...
				            type:'submit',
				            handler:function(){
//								alert(Ext.getCmp('OrderPaymentForm').form.isValid());
								if(!Ext.getCmp('OrderPaymentForm').form.isValid()){
				        	...
				        	showResult:function(btn){
				        		if(btn == 'yes'){
				        			Ext.getCmp('OrderPaymentForm').form.reset();
				        			grid.store.removeAll();
				        		}else{
				        			Ext.getCmp('OrderPaymentForm').destroy();
				        		}
				        	}
				        },{
				            text: '重置',
				            type:'reset',
				            handler:function(){Ext.getCmp('OrderPaymentForm').form.reset();}
				        }];
		OrderPaymentForm.superclass.constructor.call(this);
	},
	configComponent:function(options){
