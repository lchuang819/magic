SalesReturnForm = Ext.extend(Ext.magic.MagicForm, {
	id:'SalesReturnForm',
	title:'销售退货单',
	labelWidth:100,
	labelAlign:"right",
	...
	constructor: function(config){
		...
		var salesReturnBasicInfoFs = new Ext.form.FieldSet({
			id:'salesReturnBasicInfoFs',
			xtype: 'fieldset',
//	        title: '基本信息',
	        autoHeight:true,
	        //labelWidth:120,
//	        width:1100,
		...
		var SalesReturnGrid = Ext.extend(Ext.grid.EditorGridPanel, {
//		    renderTo: 'user-grid',
			id:'SalesReturnGrid',
		    iconCls: 'icon-grid',
		    frame: true,
...
//		        this.buttons = this.buildUI();
		        SalesReturnGrid.superclass.initComponent.call(this);
		    },
		    constructor: function(config){
...
//		 		});
		    	 ...
		    	 var colModel = [
		 		                ...
		 		                {header: "数量", sortable: true, width:30, dataIndex: 'quantity', editor: textField},
		 		                {header: "单价", sortable: true, width:30, dataIndex: 'unitPrice', editor: textField},
		 		                {header: "合计", sortable: true, width:30, dataIndex: 'totalPrice', editor: textField},
		 		                {header: "清单备注", sortable: true, dataIndex: 'itemDescription', editor: textField}
		 		            ];

		         // super
		    	 SalesReturnGrid.superclass.constructor.call(this, {store:prodCatalogStore, columns:colModel});
		    },
...
//		    },
		...
		//
		    ...
		    onAdd : function(btn, ev) {
		        ...
		        var p = new Magic.product.ProductGrid({id:'SalesReturnFormProductGrid'});
		        p.on('rowdblclick', function(cgrid, index, e){
	        		...
	        		var defaultPrice = cRecord.get('defaultPrice');
...
//		        var rec = this.store.getAt(index[0]);
			...
			doAction:function(params){
				...
				this.store.setBaseParam('orderTypeId', "SALES_RETURN_ORDER");
		...
		var grid = new SalesReturnGrid();
		
		this.items = [
		              salesReturnBasicInfoFs,
		              grid
		              ];
		...
		this.buttons = [{
				        	...
				        	showResult:function(btn){
				        		if(btn == 'yes'){
				        			Ext.getCmp('SalesReturnForm').form.reset();
				        			grid.store.removeAll();
				        		}else{
				        			Ext.getCmp('SalesReturnForm').destroy();
				        		}
				        	}
				        },{
				            text: '重置',
				            type:'reset',
				            handler:function(){Ext.getCmp('SalesReturnForm').form.reset();}
				        }];
		SalesReturnForm.superclass.constructor.call(this);
	},
	configComponent:function(options){
