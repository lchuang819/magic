	...
	PartyForm = Ext.extend(Ext.form.FormPanel, {
	    ...
	    constructor: function(config){
			...
			var partyIdTf = new Ext.form.TextField({
	    		fieldLabel: '标识',
	    		name:beanName+'.partyId',
	    		width:100,
	    		readOnly:config.enableData,
	    		value:partyId,
	    		regex:/^[a-zA-Z0-9]+$/,
...
//	        });
    		...
    		var postCommentsTf = new Ext.form.TextField({
	    		fieldLabel: '备注',
...
//	    		postalAddressFs = postalAddressGridFs;
	    	...
	    	if(config.loadParty){
	    		...
	    		items = [productBasicFs, postalAddressFs, contactMethodFs];
	    	}
	    	...
	    	PartyForm.superclass.constructor.call(this, {items:items});
		}
	});//end partyForm
	...
	function createPostalAddressGrid(){
		var store = new Ext.data.JsonStore({
	        ...
	        baseParams:{actionType:'LoadPostalAddress', partyId:globalPartyId},
	        fields: [
	            'contactMechId','toName','attnName','address1','address2','city','postalCode','countryGeoId','stateProvinceGeoId','countyGeoId'
	            // 会员标识 		用户登录 		名字		类型 
	        ]
		...
		postalAddressGrid = new Ext.grid.GridPanel({
	        ...
	        viewConfig: {
				...
				rowdblclick:function(cgrid, index, e) {
		        	...
		        	var pff = new Ext.UpdatePostalAddressForm({updateData:true,record:cRecord});
	    			var win = new Ext.CommonWindow({
	    				title:'更新',
	    				itemForm:pff,
	    				store:store,
...
//	        	}] 
	...
	function createTelecomNumberGrid(){
		var store = new Ext.data.JsonStore({
	        ...
	        remoteSort: true,
	        url: 'partyLoader.action',
	        baseParams:{actionType:'LoadTelecomNumberByParty', partyId:globalPartyId},
	        fields: [
		...
		telecomNumberGrid = new Ext.grid.GridPanel({
	        ...
	        tbar: [{
               ...
               '->',
               {
	            ...
	            iconCls:'icon-add',
	            handler : function(){
        			var telecomForm = new Ext.UpdateTelecomForm({updateData:false});
	        		var win = new Ext.CommonWindow({
	    				title:'创建',
	        ...
	        viewConfig: {
				...
				rowdblclick:function(cgrid, index, e) {
	        		...
	        		var cRecord = cgrid.store.getAt(index);
	        		var telecomForm = new Ext.UpdateTelecomForm({updateData:true,record:cRecord});
	        		var win = new Ext.CommonWindow({
	    				title:'创建',
	    				itemForm:telecomForm,
	    				store:store,
...
//	        	}] 
	...
	function createContactGrid(){
		var store = new Ext.data.JsonStore({
	        ...
	        baseParams:{actionType:'LoadContactMechByParty', partyId:globalPartyId},
	        remoteSort: true,
	        url: 'partyLoader.action',

	        fields: [
	            'contactMechId','contactMechTypeId','infoString','contactMechTypeDesc'
	            // 会员标识 		用户登录 		名字		类型 
	        ]
		...
		contactGrid = new Ext.grid.GridPanel({
	        ...
	        tbar: [{
	               ...
	               '->',
	               {
		            ...
		            handler : function(){
	        			...
	        			var contactForm = new Ext.UpdateContactForm({updateData:false});
		        		var win = new Ext.CommonWindow({
		    				title:'创建',
	        ...
	        viewConfig: {
				...
				rowdblclick:function(cgrid, index, e) {
	        		...
	        		var cRecord = cgrid.store.getAt(index);
	        		var contactForm = new Ext.UpdateContactForm({updateData:true, record:cRecord});
	        		var win = new Ext.CommonWindow({
	    				title:'更新',
	    				itemForm:contactForm,
	    				store:store,
	...
	Ext.UpdatePostalAddressForm=Ext.extend(Ext.magic.MagicForm ,{
			...
			constructor: function(config){
				...
				var provinceCbx = new Ext.magic.GeoComboBox({
					...
					useBaseParams:true,
					baseParams:{actionType:'LoadGeo',geoSecCode:'CHN', geoTypeId:'STATE'}
				});
				...
				provinceCbx.on('select', function(){
					...
					cityCbx.setValue('');
					cityCbx.store.load({params:{geoSecCode:province, geoTypeId:'CITY'}});
				});
				cityCbx.on('select', function(){
					var city = cityCbx.getValue();
					urbanCbx.setValue('');
					urbanCbx.store.load({params:{geoSecCode:city, geoTypeId:'COUNTY_CITY'}});
				});
				...
				Ext.UpdatePostalAddressForm.superclass.constructor.call(this, {items:items});
			}
		});
	...
	Ext.UpdateTelecomForm=Ext.extend(Ext.magic.MagicForm ,{
			...
			constructor: function(config){
				...
				Ext.UpdateTelecomForm.superclass.constructor.call(this, {items:items});
			}
		});
	...
	Ext.UpdateContactForm=Ext.extend(Ext.magic.MagicForm ,{
			...
			constructor: function(config){
				...
				var contactMechTypeStore = new Ext.data.JsonStore({
					...
					url: 'partyLoader.action',
					root:'dataset',
					baseParams:{actionType:'LoadContactMechType', hasTable:'N'},
					fields: [
					         'contactMechTypeId', 
					         'description',
					         'parentTypeId',
...
//					         data:[['电子邮件','EMAIL_ADDRESS'],['信息','INFO'],['错误','ERROR']]
				...
				Ext.UpdateContactForm.superclass.constructor.call(this, {items:items});
			}
		});
